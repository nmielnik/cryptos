...
const exportClosedTransactionsCSV = (bank, outputPath) => {
	const tsYear = 365 * 24 * 60 * 60 * 1000;
	const formatDate = (d) => {
	...
	const header = 'Currency Name,Purchase Date,Date Sold,Proceeds,Cost Basis,Gain/Loss,Type';
	const rows = bank.closed.map((trx) => {
		const amount = (trx['currency'] === 'BTC') ? help.satsToBTC(trx['amount']) : trx['amount'];
		const description = `${amount} ${trx['currency']}`;
		const buyDate = formatDate(trx['buy-date']);
		const sellDate = formatDate(trx['sell-date']);
		const proceeds = formatUSD(trx['sell-usd']);
		const costBasis = formatUSD(trx['buy-usd']);
		const gainLoss = formatUSD(help.fixDecimal(trx['sell-usd'] - trx['buy-usd']));
		const term = ((trx['sell-date'] - trx['buy-date']) > tsYear) ? 'Long-term' : 'Short-term';
		return `${description},${buyDate},${sellDate},${proceeds},${costBasis},${gainLoss},${term}`;
	});

	const csvContent = [header, ...rows].join('\n');
	const fs = require('fs');
	fs.writeFileSync(outputPath, csvContent, 'utf8');
	console.log(`Exported ${rows.length} closed transactions to ${outputPath}`);
};

module.exports = {
	calculateGains: calculateGains,
	printBank: printBank,
	exportClosedTransactionsCSV: exportClosedTransactionsCSV
};
