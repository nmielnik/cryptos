...
const trexApi = Promise.promisifyAll(require('node-bittrex-api'));
trexApi.options({
  'apikey' : keys.api_key,
	  ...
	  result:
	   [ { O: 0.0195422,
	     ...
	     { O: 0.01955,
	       ...
	       C: 0.01955939,
	       V: 145.08617885,
	       T: '2017-12-19T05:16:00',
	       BV: 2.83732959 }
	    ]
	}
	*/

	getMarketCandles: (coin, interval, currency) => {
		// https://bittrex.com/Api/v2.0/pub/market/GetTicks?marketName=BTC-ARK&tickInterval=oneMin
		currency = (currency || 'BTC').toUpperCase();
		interval = interval || intervalNames['5Minutes'];
		let market = `${currency}-${coin.toUpperCase()}`;
		return trexApi.getcandlesAsync({
			marketName: market,
			tickInterval: interval
		})
			.then(data => {
				if (!data.success || !data.result) {
					console.error(`Skipping ${market} due to failure: ${data.message || 'BITTREX REQUEST FAILED - NO MESSAGE'}`);
					console.error(data);
					return [];
				}
				return data.result.map(rawData => {
					return {
						'TimeStamp': rawData['T'],
						'Price': rawData['C'],
						'High': rawData['H'],
						...
						'Close': rawData['C'],
						'Volume': rawData['V'],
						'BaseVolume': rawData['BV']
					};
				});
			})
			.catch(err => {
				console.error(`Skipping ${market} due to error: ${JSON.stringify(err)}`);
				return [];
			})
	},
