			...
			trades.some(trade => {
				...
				if (mins > 0 && mins < 120) {
					...
					if (_.size(holdings.heldCoins) < tradeRules.maxCoins &&
						...
						const purchasedCoins = ((purchaseAmountInBtc * (1 - BITTREX_FEE)) / lastPrice).toFixed(8);

						console.log(`buying ${purchaseAmountInBtc} BTC of ${coin} at ${lastPrice}`);
						...
						bittrexApi.tradebuy({
						  ...
						  Target: 0, // used in conjunction with ConditionType
							...
							if (!err) {
								...
								console.log(`Error buying ${coin}: ${JSON.stringify(err)}`);
							}
						});
						...
						bittrexApi.tradesell({
							...
							if (!err) {
								...
								console.log(`Error selling ${coin}: ${JSON.stringify(err)}`);
							}
						});
...
console.log(`Starting. Trade interval=${DELAY_SECONDS_PER_ITERATION}`);
