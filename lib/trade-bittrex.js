			...
			trades.some(trade => {
				...
				if (mins > 0 && mins < 120) {
					...
					if (_.size(holdings.heldCoins) < tradeRules.maxCoins &&
						...
						((_.get(holdings.soldCoins, [coin, 'sellTime'], 0) + tradeRules.cooldown) <= iter) // cooldown is expired or coin was
						) {
						const purchaseAmountInBtc = holdings.free / (tradeRules.maxCoins - _.size(holdings.heldCoins));
						const purchasedCoins = parseInt((purchaseAmountInBtc / lastPrice).toFixed(8));
						...
						bittrexApi.tradebuy({
						  ...
						  Target: 0, // used in conjunction with ConditionType
							...
							if (!err) {
								...
								holdings.free = Math.Max(holdings.free - parseInt((purchaseAmountInBtc * (1 + BITTREX_FEE)).toFixed(8)), 0);
						...
						bittrexApi.tradesell({
							...
							if (!err) {
								...
								holdings.profit = holdings.profit + holdings.soldCoins[coin].amount * (lastPrice - holdings.soldCoins[coin].buyPrice) * (1 - BITTREX_FEE);
					...
					if (pctChg > 0 && (absChg >= tradeRules.thresholdPct && mins >= tradeRules.minThresholdMins && mins <= tradeRules.maxThresholdMins)) {
						swings.push({
							coin: coin,
							change: pctChg,
							minutes: mins
