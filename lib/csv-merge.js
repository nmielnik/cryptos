...
const geminiGetAmount = (data, currency, isFee, isStaking) => {
	let colName = (isStaking) ? `Amount ${currency}` : `${currency} Amount ${currency}`;
	if (isFee) {
...
const krakenCostMarkets = ['USDC', 'XBT', 'ETH', 'USD', 'BTC'];
...
const converters = {
	...
	'gemini': (data) => {
		...
		if (!trans.date.getFullYear()) {
			console.error('@@@ BAD GEMINI DATE: ' + data['Date'] + ' || ' + data['Time (UTC)']);
			console.error(data);
		}
	...
	'gemini-staking': (data) => {
		...
		if (transType == 'Deposit' ||
			transType == 'Monthly Interest Summary' ||
			transType == 'Redeem') {
			return null;
		} else if (transType == 'Interest Credit') {
			trans['type'] = 'interest';
			trans['currency'] = data['Symbol'];
			trans['amount'] = trans['net-amount'] = geminiGetAmount(data, trans['currency'], false, true);
			trans['amount-usd'] = geminiGetAmount(data, 'USD', false, true);
			if (!trans['amount-usd']) {
				// Ignore interest earned that's less the $0.01
				return null;
			}
		} else {
			console.error('UNKNOWN TYPE: ' + transType);
	...
	'kraken': (data) => {
		...
		if (!krakenCostMarkets.some((str) => {
			if (pairStr.endsWith(str)) {
				...
				if (pair.left.endsWith('/')) {
					pair.left = pair.left.substring(0, pair.left.length - 1);
				}
				return true;
			}
			return false;
	...
	'manual-transactions': (data) => {
		const trans = {
			date: moment.utc(data['Date'], 'YYYY-MM-DD HH:mm:ss').toDate(),
			exchange: data['Exchange'],
			description: data['Description']
		};
		if (!trans.date.getFullYear()) {
			console.error('@@@ BAD MANUAL DATE: ' + data['Date'] + ' || ' + data['Time (UTC)']);
			console.error(data);
		}
		const transType = data['Type'];
		if (transType == 'Buy') {
			const assetCurr = data['Currency'];
			const costCurr = data['Other Currency'];

			trans['type'] = 'trade';
			trans['trade-type'] = 'buy';
			trans['currency'] = assetCurr;
			trans['amount'] = trans['net-amount'] = help.parseAmount(data['Amount'], assetCurr);

			trans['cost-currency'] = costCurr;
			trans['fee-currency'] = costCurr;
			trans['fee'] = 0;
			trans['cost'] = trans['net-cost'] = help.parseAmount(data['Other Amount'], costCurr);
			...
			const costCurr = data['Currency'];
			const assetCurr = data['Other Currency'];

			trans['type'] = 'trade';
			trans['trade-type'] = 'sell';
			trans['currency'] = assetCurr;
			trans['amount'] = trans['net-amount'] = help.parseAmount(data['Other Amount'], assetCurr);
			trans['fee-currency'] = assetCurr;
			trans['fee'] = 0;

			trans['cost-currency'] = costCurr;
			trans['cost'] = trans['net-cost'] = Math.abs(help.parseAmount(data['Amount'], costCurr));
...
const csvs2024 = [
	{ file: 'local/2024/cryptocomHistory-2024.csv', conv: 'cryptocom'},
	{ file: 'local/2024/GeminiHistory-2024.csv', conv: 'gemini'},
	{ file: 'local/2024/GeminiStaking-2024.csv', conv: 'gemini-staking' },
	{ file: 'local/2024/Kraken-Trades-2024.csv', conv: 'kraken' },
	{ file: 'local/2024/ManualTransactions-2024.csv', conv: 'manual-transactions' },
];

const csvs2025 = [
	{ file: 'local/2025/GeminiHistory-2025.csv', conv: 'gemini'},
	{ file: 'local/2025/cash_app_btc_report-2025.csv', conv: 'cashapp' },
];
...
const getGainsTransactions = (year) => {
	...
	if (year >= 2024)
		csvList = csvList.concat(csvs2024);
	if (year >= 2025)
		csvList = csvList.concat(csvs2025);

	console.log(`Processing ${csvList.length} CSVs for ${year}`);
	const csvStatsToShow = csvs2025;
	...
	.then((groups) => {
		...
		groups.forEach((group) => {
			parts.push(group.results);
			if (csvStatsToShow.find((pair) => pair.file == group.stats.file )) {
