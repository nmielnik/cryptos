...
const converters = {
	...
	'gemini': (data) => {
		...
		if (transType == 'Buy') {
			...
			const costCurr = symbol.substring(symbol.length - 3);
			const assetCurr = symbol.substring(0, symbol.length - 3);
	...
	'nexo': (data) => {
		...
		if (transType == 'Withdrawal') {
			trans['type'] = 'withdrawal';
			trans['currency'] = data['Currency'];
			trans['net-cost'] = trans['amount'] = Math.abs(help.parseAmount(data['Amount'], trans['currency']));
			...
			trans['currency'] = data['Currency'];
			trans['amount'] = trans['net-amount'] = Math.abs(help.parseAmount(data['Amount'], trans['currency']));
			trans['amount-usd'] = help.parseAmount(data['USD Equivalent'], 'USD');
		} else {
			console.error(`UNKNOWN TYPE: ${transType}`);
			return null;
...
const get2021GainsTransactions = () => {
	return Promise.all([convCSV.convertCSV('local/2021/cash_app_report.csv', converters['cashapp']),
					convCSV.convertCSV('local/2021/GeminiHistory-Export.csv', converters['gemini']),
