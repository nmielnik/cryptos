...
const printBank = (bank) => {
	...
	console.log(`Total Gains: $${bank.gains} | Proceeds: ($${bank.shortProceeds}) - Cost Basis: ($${bank.shortBasis})`);
...
const removeBasis = (bank, toRemove) => {
	...
	if (toRemove['remaining'] > 0) {
		if (calculateGains.debug)
			console.error(`Unacccounted cost basis: ${toRemove['remaining']} ${curr}`);
		if (!curr) {
